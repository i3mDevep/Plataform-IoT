...
const usernameArgs = {
  where: { username: 'i3m', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.i3m))
...
test.serial('Agent#findByUsername', async t => {
  const agents = await db.Agent.findByUsername('i3m')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.i3m.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.i3m, 'agents should be the same')
})
